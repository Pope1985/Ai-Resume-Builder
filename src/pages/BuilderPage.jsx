...
import {
  Sparkles, User, Briefcase, GraduationCap, Code2, Download,
  Plus, ChevronRight, Heart, Folder, Save, Palette, X,
  CheckCheck, Loader2, Eye, EyeOff,
} from "lucide-react";
...
const inputCls = `w-full bg-[#0f0f12] border border-white/8 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/60 focus:bg-[#13131a] transition-all duration-200`;
  ...
  return (
    <div style={{ fontFamily: "'Times New Roman', Times, serif" }} className="text-black px-6 sm:px-12 py-8 sm:py-10">
      <div className="text-center mb-2">
        ...
        {resume.title && <p className="text-sm font-bold uppercase tracking-widest mt-1">{resume.title}</p>}
      </div>
      ...
      {resume.skills && (
        <section className="mb-5">
          ...
          <ul className="list-disc list-outside pl-5 text-xs sm:text-sm space-y-1">
            {resume.skills.split(",").map((s) => s.trim()).filter(Boolean).map((s, i) => <li key={i}>{s}</li>)}
          </ul>
        </section>
      )}
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif" }} className="text-black">
      <div className="bg-gradient-to-r from-violet-600 to-indigo-600 px-6 sm:px-10 py-8 sm:py-10">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs sm:text-sm text-violet-100">
      ...
      <div className="px-6 sm:px-10 py-6 sm:py-8 space-y-6 bg-gradient-to-br from-slate-50 to-slate-100">
        {resume.experience?.some((e) => e.company || e.role) && (
          <section>
            <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Experience</span><div className="flex-1 h-px bg-gray-200" /></div>
            {resume.experience.filter((e) => e.company || e.role).map((exp, i) => (
              <div key={i} className="mb-5">
                <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                  <div><p className="text-sm font-semibold text-gray-900">{exp.role || "—"}</p><p className="text-xs sm:text-sm text-violet-600">{exp.company}</p></div>
                ...
                {exp.bullets && <ul className="mt-2 space-y-1.5">{exp.bullets.split("\n").filter(Boolean).map((b, j) => <li key={j} className="flex gap-2 text-xs sm:text-sm text-gray-600"><span className="text-violet-400 mt-1 shrink-0">▸</span>{b}</li>)}</ul>}
              </div>
            ))}
          </section>
        )}
        {resume.projects?.some((p) => p.title) && (
          <section>
            <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Projects</span><div className="flex-1 h-px bg-gray-200" /></div>
            {resume.projects.filter((p) => p.title).map((proj, i) => (
              <div key={i} className="mb-4">
        ...
        {resume.skills && (
          <section>
            <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Skills</span><div className="flex-1 h-px bg-gray-200" /></div>
            <div className="flex flex-wrap gap-2">{resume.skills.split(",").map((s) => s.trim()).filter(Boolean).map((skill, i) => <span key={i} className="text-xs text-gray-700 bg-white border border-gray-200 px-3 py-1.5 rounded-lg font-medium">{skill}</span>)}</div>
          </section>
        )}
        {resume.education?.some((e) => e.school) && (
          <section>
            <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Education</span><div className="flex-1 h-px bg-gray-200" /></div>
            {resume.education.filter((e) => e.school).map((edu, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:justify-between mb-3 gap-1">
                <div><p className="text-sm font-semibold text-gray-900">{edu.school}</p><p className="text-xs sm:text-sm text-gray-500">{edu.degree}</p></div>
        ...
        {resume.hobbies && (
          <section>
            <div className="flex items-center gap-3 mb-4"><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">Interests</span><div className="flex-1 h-px bg-gray-200" /></div>
            <div className="flex flex-wrap gap-2">{resume.hobbies.split(",").map((h) => h.trim()).filter(Boolean).map((hobby, i) => <span key={i} className="text-xs text-gray-700 bg-white border border-gray-200 px-3 py-1.5 rounded-lg">{hobby}</span>)}</div>
          </section>
        )}
      </div>
    </div>
...
function MinimalTemplate({ resume }) {
  return (
    <div style={{ fontFamily: "Georgia, serif" }} className="text-black bg-white px-6 sm:px-10 py-8 sm:py-10">
      ...
      {resume.title && <p className="text-sm text-gray-500 mt-1">{resume.title}</p>}
      <p className="text-xs sm:text-sm text-gray-500 mt-2 break-words">{[resume.email, resume.location, resume.phone].filter(Boolean).join(" | ")}</p>
      <div className="border-t-2 border-gray-800 mt-5 mb-6" />
      {resume.summary && <section className="mb-6"><div className="border-l-4 border-gray-800 pl-3 mb-3"><h2 className="text-xs font-bold uppercase tracking-widest text-gray-500">Summary</h2></div><p className="text-xs sm:text-sm leading-relaxed text-gray-700">{resume.summary}</p></section>}
      {resume.experience?.some((e) => e.company || e.role) && (
        <section className="mb-6">
          <div className="border-l-4 border-gray-800 pl-3 mb-3"><h2 className="text-xs font-bold uppercase tracking-widest text-gray-500">Experience</h2></div>
          {resume.experience.filter((e) => e.company || e.role).map((exp, i) => (
            <div key={i} className="mb-4">
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1"><p className="text-xs sm:text-sm font-bold text-gray-900">{exp.role || "—"}</p><span className="text-xs sm:text-sm text-gray-500">{exp.duration}</span></div>
              <p className="text-xs sm:text-sm text-gray-500">{exp.company}</p>
              {exp.bullets && <ul className="mt-2 list-disc list-outside pl-5 text-xs sm:text-sm text-gray-700 space-y-1">{exp.bullets.split("\n").filter(Boolean).map((b, j) => <li key={j}>{b}</li>)}</ul>}
            </div>
          ))}
        </section>
      )}
      {resume.skills && <section className="mb-6"><div className="border-l-4 border-gray-800 pl-3 mb-3"><h2 className="text-xs font-bold uppercase tracking-widest text-gray-500">Skills</h2></div><p className="text-xs sm:text-sm text-gray-700">{resume.skills.split(",").map((s) => s.trim()).filter(Boolean).join(" • ")}</p></section>}
      {resume.education?.some((e) => e.school) && (
        <section className="mb-6">
          <div className="border-l-4 border-gray-800 pl-3 mb-3"><h2 className="text-xs font-bold uppercase tracking-widest text-gray-500">Education</h2></div>
          {resume.education.filter((e) => e.school).map((edu, i) => (
            <div key={i} className="flex flex-col sm:flex-row sm:justify-between mb-2 gap-1">
              <div><p className="text-xs sm:text-sm font-bold text-gray-900">{edu.school}</p><p className="text-xs sm:text-sm text-gray-500">{edu.degree}</p></div>
              <span className="text-xs sm:text-sm text-gray-500">{edu.year}</span>
            </div>
          ))}
        </section>
      )}
      {resume.hobbies && <section className="mb-6"><div className="border-l-4 border-gray-800 pl-3 mb-3"><h2 className="text-xs font-bold uppercase tracking-widest text-gray-500">Interests</h2></div><p className="text-xs sm:text-sm text-gray-700">{resume.hobbies}</p></section>}
    </div>
  );
}
...
function AiModal({ suggestion, onApply, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm px-0 sm:px-4">
      <div className="w-full sm:max-w-lg bg-[#0f0f14] border border-white/10 rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-violet-500/20 border border-violet-500/30 flex items-center justify-center"><Sparkles size={13} className="text-violet-400" /></div>
            <span className="text-sm font-semibold text-white/90">AI Suggestion</span>
          </div>
          <button onClick={onClose} className="text-white/30 hover:text-white/70 transition p-1 rounded-lg"><X size={16} /></button>
        </div>
        <div className="px-6 py-4">
          <p className="text-[11px] uppercase tracking-widest text-white/25 mb-3 font-semibold">Generated output</p>
          <div className="bg-[#0c0c10] border border-white/6 rounded-xl px-4 py-3 text-sm text-white/80 leading-relaxed whitespace-pre-wrap max-h-48 sm:max-h-64 overflow-y-auto">{suggestion}</div>
        </div>
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onApply} className="flex-1 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 px-4 py-2.5 rounded-xl text-sm font-semibold transition"><CheckCheck size={14} />Apply to field</button>
          <button onClick={onClose} className="flex items-center justify-center gap-2 bg-white/6 hover:bg-white/10 border border-white/8 px-4 py-2.5 rounded-xl text-sm font-semibold text-white/60">Dismiss</button>
        </div>
      </div>
    </div>
  );
...
export default function BuilderPage() {
  ...
  const saveResume = () => {
    ...
    const savedId = existing ? existing.id : Date.now();
    const newResume = {
      id: savedId,
  ...
  const handleSaveToDashboard = () => { saveResume(); navigate("/dashboard"); };
  const handleExportPDF = () => { const savedId = saveResume(); navigate(`/preview/${savedId}`); };
  ...
  return (
    <div className="h-screen bg-[#080809] text-white flex flex-col overflow-hidden">
      ...
      {/* Header */}
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/6 shrink-0 bg-[#080809]/80 backdrop-blur-sm gap-3">
        <div className="flex items-center gap-2 shrink-0">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-violet-500/20 border border-violet-500/30 flex items-center justify-center">
        ...
        <div className="flex gap-2">
          <button onClick={handleSaveToDashboard} className="flex items-center gap-1.5 bg-violet-600 hover:bg-violet-500 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition shadow-lg shadow-violet-500/20">
            <Save size={13} />
            <span className="hidden xs:inline">Save</span>
          </button>
          <button onClick={handleExportPDF} className="flex items-center gap-1.5 bg-white text-gray-900 hover:bg-gray-100 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition shadow-lg">
            <Download size={13} />
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <nav className={`w-full lg:w-52 shrink-0 border-b lg:border-b-0 lg:border-r border-white/6 p-2 sm:p-3 flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-y-auto ${showPreview ? "hidden lg:flex" : "flex"}`}>
          {SECTIONS.map(({ id, icon: Icon, desc }) => (
            <button key={id} onClick={() => setActive(id)} className={`group flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 sm:py-3 rounded-xl text-left transition-all duration-200 min-w-max lg:min-w-0 ${active === id ? "bg-violet-500/15 border border-violet-500/20 text-white" : "text-white/40 hover:text-white/70 hover:bg-white/4 border border-transparent"}`}>
              <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center shrink-0 ${active === id ? "bg-violet-500/30" : "bg-white/6"}`}>
                <Icon size={12} />
              </div>
              <div className="min-w-0">
                <div className="text-xs sm:text-sm font-medium leading-tight">{id}</div>
                <div className="text-[9px] sm:text-[10px] text-white/25 mt-0.5 leading-tight truncate hidden sm:block">{desc}</div>
              </div>
              {active === id && <ChevronRight size={11} className="ml-auto text-violet-400 shrink-0 hidden lg:block" />}
            </button>
          ))}
        </nav>
        ...
        <main className={`w-full lg:w-[320px] shrink-0 border-b lg:border-b-0 lg:border-r border-white/6 overflow-y-auto p-4 sm:p-5 flex flex-col gap-4 sm:gap-5 ${showPreview ? "hidden lg:flex" : "flex"}`}>
          ...
          {active === "Experience" && (
            <>
              <div><h2 className="text-base font-semibold text-white/90">Work experience</h2><p className="text-xs text-white/30 mt-0.5">Add your roles, latest first</p></div>
              ...
              <button onClick={() => update("experience", [...resume.experience, { company: "", role: "", duration: "", bullets: "" }])} className="flex items-center gap-2 text-sm text-violet-400 hover:text-violet-300 transition py-1"><Plus size={14} /> Add another role</button>
            </>
          )}

          {active === "Education" && (
            <>
              <div><h2 className="text-base font-semibold text-white/90">Education</h2><p className="text-xs text-white/30 mt-0.5">Schools and qualifications</p></div>
              ...
              <button onClick={() => update("education", [...resume.education, { school: "", degree: "", year: "" }])} className="flex items-center gap-2 text-sm text-violet-400 hover:text-violet-300 transition py-1"><Plus size={14} /> Add another</button>
            </>
          )}
            ...
            <>
              ...
              {resume.skills && (
                <div className="flex flex-wrap gap-2 p-4 rounded-2xl bg-white/3 border border-white/6">
                  {resume.skills.split(",").map((s) => s.trim()).filter(Boolean).map((s, i) => <span key={i} className="bg-violet-500/15 border border-violet-500/20 text-violet-300 text-xs px-3 py-1.5 rounded-lg">{s}</span>)}
                </div>
              )}
            </>
          )}
          ...
          {active === "Projects" && (
            <>
              <div><h2 className="text-base font-semibold text-white/90">Projects</h2><p className="text-xs text-white/30 mt-0.5">Showcase your work</p></div>
              ...
              <button onClick={() => update("projects", [...resume.projects, { title: "", description: "", link: "" }])} className="flex items-center gap-2 text-sm text-violet-400 hover:text-violet-300 transition py-1"><Plus size={14} /> Add another project</button>
            </>
          )}
          ...
          {active === "Hobbies" && (
            <>
              ...
              {resume.hobbies && (
                <div className="flex flex-wrap gap-2 p-4 rounded-2xl bg-white/3 border border-white/6">
                  {resume.hobbies.split(",").map((h) => h.trim()).filter(Boolean).map((hobby, i) => <span key={i} className="bg-violet-500/15 border border-violet-500/20 text-violet-300 text-xs px-3 py-1.5 rounded-lg">{hobby}</span>)}
                </div>
              )}
            </>
          )}
          ...
          {active === "Templates" && (
            <>
              <div><h2 className="text-base font-semibold text-white/90">Templates</h2><p className="text-xs text-white/30 mt-0.5">Choose a resume style</p></div>
        ...
        {/* Preview */}
        <section className={`flex-1 overflow-y-auto bg-[#0c0c0f] items-start justify-center p-4 sm:p-8 ${showPreview ? "flex" : "hidden lg:flex"}`}>
          <div
            ref={previewRef}
